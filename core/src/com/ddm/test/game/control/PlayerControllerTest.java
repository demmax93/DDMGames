 ...
 * 27.08.2017
 */
public class PlayerControllerTest implements InputProcessor {
    private final static String TAG = PlayerControllerTest.class.getSimpleName();
    ...
    private static Map<Keys, Boolean> keys = new HashMap<PlayerControllerTest.Keys, Boolean>();
    ...
    public PlayerControllerTest(Entity player){
    ...
    private void processInput(float delta){
        ...
        if (keys.get(Keys.DOWN ) || keys.get(Keys.LEFT) || keys.get(Keys.RIGHT) || keys.get(Keys.UP)){
       ...
       /* if( keys.get(Keys.LEFT)){
            player.calculateNextPosition(Entity.Direction.LEFT,
                    delta);
            player.setState(Entity.State.WALKING);
            player.setDirection(Entity.Direction.LEFT, delta);
        } else if( keys.get(Keys.RIGHT)){
            player.calculateNextPosition(Entity.Direction.RIGHT,
                    delta);
            player.setState(Entity.State.WALKING);
            ...
            player.calculateNextPosition(Entity.Direction.DOWN,
                    delta);
            player.setState(Entity.State.WALKING);
